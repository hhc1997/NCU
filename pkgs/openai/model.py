...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.n_ctx = 4
        #self.compound_prompts_depth = 1 # max=12, but will create 11 such shared prompts
        ...
        #     ("linear2", nn.Linear(embed_dim // 16, transformer_width))
        # ]))
    ...
    def get_text_features(self, input_caps=None, processor=None, device=None, mode='yes', img_features = None):
        if mode == "yes":
            ...
            prompts = [self.prompt_prefix + " " + random.choice(self.no_text) + " " + cap for cap in input_caps]
            ...
            # bias = self.prompt_meta_net(img_features)
    ...
    def forward(self, raw_caps, processor, pixel_values, device, cap_aug = False):
        ...
        if cap_aug == True:
            ...
            text_features_no = self.get_text_features(input_caps=aug_caps, processor=processor, device=device,
                                                      mode='no', img_features = image_features)
        else:
