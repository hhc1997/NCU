...
class Processor:
    ...
    #     [Resize(224, interpolation=Image.BICUBIC), CenterCrop(model.visual.input_resolution),
    ...
    def process_image(self, image, train_mode = False):
        if train_mode:
            return self.transform_tain(image.convert("RGB"))
        else:
            return self.transform_val(image.convert("RGB"))
...
def load(name, pretrained = False):
    ...
    try:
        model = build_ori(model.state_dict(), pretrained = pretrained)
    except KeyError:
        state_dict = {key["module.":]: value for key, value in state_dict["state_dict"].items()}
        model = build_ori(state_dict, pretrained = pretrained)
...
def load_processor(model):
    """
    Load a CLIP processor without loading the model.
    Since all CLIP models (ViT-B/32, ViT-B/16, etc.) use the same processor,
    we don't need a name parameter.

    Returns:
        Processor: A processor instance for text and image preprocessing
    """
