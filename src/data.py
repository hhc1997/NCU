...
class ImageRawCaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        item["pixel_values"] = self.processor.process_image(Image.open(os.path.join(self.root, self.images[idx])), train_mode=True)
        return item
...
def get_train_dataloader(options, processor):
    ...
    dataset = ImageRawCaptionDataset(path, image_key = options.image_key, caption_key = options.caption_key, delimiter = options.delimiter, processor = processor)
        
    sampler = DistributedSampler(dataset) if(options.distributed) else None


    dataloade_CLIP = DataLoader(dataset, batch_size = batch_size, shuffle = False, num_workers = options.num_workers, pin_memory = True, sampler = sampler, drop_last = True, persistent_workers = True, prefetch_factor=4)
...
def get_eval_test_dataloader(options, processor):
    ...
    dataloader = torch.utils.data.DataLoader(dataset, batch_size = options.batch_size, num_workers = options.num_workers, sampler = None, pin_memory = True,
                                             prefetch_factor=4)
