...
def get_validation_metrics(model, dataloader, processor, options):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, ascii=True):
            ...
            outputs = model(raw_caps=raw_captions, processor=processor, pixel_values=pixel_values,
            ...
            target = torch.arange(len(raw_captions)).long().to(options.device, non_blocking=True)
...
def get_zeroshot_metrics(model, processor, test_dataloader, options):
    logging.info(f"Started zeroshot testing data num:{len(test_dataloader.dataset)}")
    model.eval()
    umodel = model.module if (options.distributed) else model
    ...
    with torch.no_grad():
        ...
        for c in tqdm(classes, ascii=True):
            ...
            text_embedding = umodel.get_text_features(raw_caps=text, processor=processor, device=options.device)
    ...
    with torch.no_grad():
        ...
        for image, label in tqdm(test_dataloader, ascii=True):
...
def get_linear_probe_metrics(model, train_dataloader, test_dataloader, options):
    ...
    labels = None
    with torch.no_grad():
        for image, label in tqdm(train_dataloader, ascii=True):
    ...
    try:
        ...
        input_dim = umodel.text_projection.out_features
    ...
    optimizer = optim.AdamW([{"params": [parameter for name, parameter in classifier.named_parameters() if
                                         (("bias" in name) and parameter.requires_grad)], "weight_decay": 0}, {
                                 "params": [parameter for name, parameter in classifier.named_parameters() if
                                            (("bias" not in name) and parameter.requires_grad)], "weight_decay": 0.01}])
    scheduler = cosine_scheduler(optimizer, 3e-4, 0, len(train_dataloader) * options.linear_probe_num_epochs)
    criterion = nn.CrossEntropyLoss().to(options.device)

    pbar = tqdm(range(options.linear_probe_num_epochs), ascii=True)
    for epoch in pbar:
        cbar = tqdm(train_dataloader, leave=False, ascii=True)
